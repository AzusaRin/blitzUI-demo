...
.bl-button {
  ...
  text-align: center;cursor: pointer;transition: .1s;display: inline-flex;justify-content: center;align-items: center;
  vertical-align: middle;

  &:hover {
    color: var(--button-color-hover);border-color: var(--button-border-color-hover);background-color: var(--button-bg-hover);

    > .icon {
      fill: var(--button-color-hover);animation: jump 1s ease;
    }
    &.icon-right {
      > .icon {
  ...
  &:active {
    color: var(--button-color-border-active);border-color: var(--button-color-border-active);
  }
  ...
  &.icon-right {
    ...
    > .loading{order: 2;margin-left: 0.3em;margin-right: 0;
    }
    > .content {order: 1;
    }
  }
